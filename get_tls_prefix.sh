...
function exec_process () {
    ...
    psql -h $PGHOST -p $PGPORT -d $PGDATABASE -U $USER -t -A -F"," -c "SELECT accession FROM accession WHERE accession LIKE 'TA%' ORDER BY accession DESC LIMIT 1;" | sed -E 's/[^A-Za-z]//g' > ~/temp_sakura2DB/last_prefix_actual_conv.txt
    # umss
    psql -h $PGHOST -p $PGPORT -d $PGDATABASE -U $USER -t -A -F"," -c "SELECT DISTINCT prefix,set_version FROM umss_ann_entry WHERE accession LIKE 'TA%' ORDER BY prefix DESC LIMIT 1;" | awk '{sub(",",""); print $1$2}' > ~/temp_sakura2DB/last_prefix_actual_umss.txt
    # -test
    PGDATABASE="e-test"
    PGHOST="a012"
    psql -h $PGHOST -p $PGPORT -d $PGDATABASE -U $USER -t -A -F"," -c "SELECT accession FROM accession WHERE accession LIKE 'TA%' ORDER BY accession DESC LIMIT 1;" | cut -c1-6 > ~/temp_sakura2DB/last_prefix_test_conv.txt
    # umss
    psql -h $PGHOST -p $PGPORT -d $PGDATABASE -U $USER -t -A -F"," -c "SELECT DISTINCT prefix,set_version FROM umss_ann_entry WHERE accession LIKE 'TA%' ORDER BY prefix DESC LIMIT 1;" | awk '{sub(",",""); print $1$2}' > ~/temp_sakura2DB/last_prefix_test_umss.txt
    # select last prefix
    ...
    if [[ ${form_ans1} = "Y" ]]; then
      awk -v var1="$last_prefix" -v var2="$lines" '$1 == var1 {for(i=1; i<=var2; i++) {getline; print $1 "01"}}' ${BASE}/tsunami/tls_all.txt > ~/temp_sakura2DB/next_prefix.txt
...
if [[ "$CURRENT_HOST" == "$EXEC_HOST1" ]]; then
    ...
    echo "This is TLS data and here is $CURRENT_HOST, please login on tsunami-adenine."
fi
