...
autofix_overlap <- function() {
  ...
  feature_location <- read.table("/data/Rfixed/feature_location.tsv", header = T, sep = "\t") # previously temp_autofix
  ...
  feature_location$assembly_gap_location <- paste(feature_location$gap_start, feature_location$gap_end, sep = "..")
  feature_location <- feature_location[, !colnames(feature_location) %in% c("gap_start", "gap_end")]
  feature_location$temp_cds <- paste(feature_location$cds_start, feature_location$cds_end, sep = "..")

  # feature_location <- feature_location %>%
  #   unite("assembly_gap_location", gap_start, gap_end, sep = "..", remove = TRUE)
  # feature_location <- feature_location %>%
  #   mutate(temp_cds = paste(cds_start, cds_end, sep = ".."))
  ...
  if(len_unique_gap_overlap > 0) { # check if there are no entry with count == 1
    ...
    feature_location <- feature_location[feature_location$len_cds1 > 50 & feature_location$len_cds2 > 50,]
