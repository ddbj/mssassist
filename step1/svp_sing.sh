...
while IFS= read -r filename; do
  ...
  biosample_id=`awk -F"\t" '$4=="biosample" {print $5}' /data/temp_svp/common2.txt`
  ...
  # add exact number!
  if [ -f /data/temp_svp/temp_main_table.txt ] && [ -s /data/temp_svp/temp_main_table.txt ]; then
 ...
 echo "No assembly_gap"
  ...
  /mnt/format_fasta ${fasta_filename} # v2.9
  ...
  if [ -f /data/temp_svp/temp_partial_rrna.txt ] && [ -s /data/temp_svp/temp_partial_rrna.txt ]; then
    ...
    echo "No need to modify the location of rRNA features output from DFAST."
  fi
  # SVP0200; CDS/mRNA; v3.4
  awk -F"\t" -v OFS="\t" '$2 == "source" { entry = $1 }($2 == "CDS" || $2 == "mRNA") { type = $2 } $4 == "locus_tag" && (type == "CDS" || type == "mRNA") {print entry, type, $5 
  type = ""  }' $ann_filename > /data/temp_svp/temp_cds_mrna.txt
  # awk -F"\t" -v OFS="\t" '{ if($2 == "source"){entry = $1}; if($2 == "CDS" || $2 == "mRNA"){type = $2}; if($4 == "locus_tag") {print entry, type, $5 }}' $ann_filename > /data/temp_svp/temp_cds_mrna.txt
  if [ -f /data/temp_svp/temp_cds_mrna.txt ] ; then
    Rscript /mnt/check_cds_mrna.R /data/temp_svp/temp_cds_mrna.txt $ann_filename
  fi
  # SVP0210: Rule /chromosome qualifiers values
  awk -F"\t" -v OFS="\t" '$2 == "source" { entry = $1 }($4 == "chromosome") {print entry, $4, $5 }' $ann_filename > /data/temp_svp/temp_chromosome_qualifier.txt
  if [ -f /data/temp_svp/temp_chromosome_qualifier.txt ] ; then
    echo "Check SVP0210"
    Rscript /mnt/check_chr_qualifier.R $ann_filename
  fi

  # SVP0110: function intron_length less than 11 bases (added locus_tag: from v0.6); removing rule SVP0110
  # awk -F"\t" -v OFS=";" '{if($2 == "source"){entry = $1}; if($2 == "CDS" || $2 == "mRNA") {print entry, $2, $4, $5, $3}}' $ann_filename > /data/temp_svp/temp_join1.txt # 
  awk -F"\t" -v OFS=";" '{if($2 == "source"){entry = $1}; if($2 == "CDS" || $2 == "mRNA") {print entry, $2, $3}}' $ann_filename > /data/temp_svp/temp_join1.txt # v3.3 ; removed product name
  
  if [ -f /data/temp_svp/temp_join1.txt ] && [ -s /data/temp_svp/temp_join1.txt ]; then
    ...
    if [ -f /data/temp_svp/temp_join2.txt ] && [ -s /data/temp_svp/temp_join2.txt ]; then
      awk -F";" '{gsub("%%", "\n"$1";"$2";"); print}' /data/temp_svp/temp_join2.txt | fgrep "," > /data/temp_svp/join2.txt
      awk '{gsub(";",","); print}' /data/temp_svp/join2.txt | awk -F"," -v OFS="\t" '{print $1, $2, $3","$4, ($4-1)-($3+1)+1}' | awk -F"\t" -v  var1="$biosample_id" -v var2="$ann_filename" -v OFS="\t" '$4 < 10 {print var2, "warning", "SVP0110", var1, $1, $2, $3, "intron length = " $4}' > /data/temp_svp/warning_intron_length2.b9
      echo "warning_intron_length2 generated"

      if [ -f /data/temp_svp/warning_intron_length2.b9 ] && [ -s /data/temp_svp/warning_intron_length2.b9 ]; then
        echo "warning_intron_length2 exists"
        ...
        echo "warning_intron not generated."
      fi

    else
      echo "check point 138."
    fi
  
  else
    echo "check point 142."
  fi
  
  # SVP0141: Detect artificial_location. Warning intron_len < 10 bases and artificial_location qualifier.
  ...
  if [ -f /data/temp_svp/temp_artificial_location.txt ] && [ -s /data/temp_svp/temp_artificial_location.txt ] && [ -f /data/temp_svp/temp_intron_length2.txt ] && [ -s /data/temp_svp/temp_intron_length2.txt ]; then
      awk -F";" 'FNR==NR{a[$1]=$1;next} ($1 in a) {print $2}' /data/temp_svp/temp_artificial_location.txt /data/temp_svp/temp_intron_length2.txt | awk '{sub("SVP0110", "SVP0141"); print $0 }' > /data/temp_svp/warning_artificial_location.b9
        # SVP0140: Detect artificial_location. Warning intron_len < 10 bases and NO artificial_location qualifier.
      if [ -f /data/temp_svp/temp_intron_length_locus_tag.txt ] && [ -s /data/temp_svp/temp_intron_length_locus_tag.txt ]; then
  ...
  sh /mnt/ddbj_filetype4svp_sing.sh
  ...
  if [ ${len_filetype} -eq 1 ]; then
    ...
    if [ ${filetype} = "TSA" ]; then
      ...
      Rscript --vanilla /mnt/cds_partial_location.R ${ann_filename}
         ...
         if [ ${len_organelle} -eq 0 ]; then
            ...
            if test -f "/data/temp_svp/temp_gt_ag_rule.txt"; then
              ...
              if [ ${len_gt_ag_rule} -gt 0 ]; then
                ...
                Rscript --vanilla /mnt/rule_gtag_sing.R
                if ls /data/temp_svp/temp_out/error_trad359_* 1> /dev/null 2>&1; then
