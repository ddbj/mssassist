...
suppressMessages(require(tidyr, quietly = T))
...
ddbj_mss_validation <- function() {
  ...
  if(lines_input_ids > 0) {
    input_ids <- read.table("/data/temp_svp/input_ids.txt", header=F)
    ...
    for(l in 1:len_input_ids) {
      ...
      if(len_ann_biosample_id > 0) {
        ...
        sample_smp_id <- bs[bs$accession_id == ann_biosample_id,] # grep
        if (exists("sample_smp_id") && !is.null(sample_smp_id) && nrow(sample_smp_id) > 0) { # v.1.7
          ...
          if(lines_feature_assembly_gap > 0 & lines_feature_cds > 0) {
            ...
            feature_cds <- fread("/data/temp_autofix/feature_cds.tsv", header = F, sep = "\t")
            colnames(feature_assembly_gap) <- c("Entry", "assembly_gap_location")
            colnames(feature_cds) <- c("Entry", "type", "cds_location")
            feature_location <- merge(feature_cds, feature_assembly_gap, by = "Entry", allow.cartesian=TRUE) 
            feature_location$temp_cds <- sub("complement","",feature_location$cds_location)
            feature_location$temp_cds <- sub("join","",feature_location$temp_cds) # v1.8
            feature_location$temp_cds <- gsub("[(]","",feature_location$temp_cds)
            feature_location$temp_cds <- gsub("[)]","",feature_location$temp_cds)
            ## remove < and > signals
            feature_location$temp_cds <- sub(">","",feature_location$temp_cds)
            feature_location$temp_cds <- sub("<","",feature_location$temp_cds)
            feature_location <- feature_location %>% separate_rows(temp_cds, sep = ",") # I am here
            # v1.8
            feature_location <- feature_location %>%
              filter(str_detect(temp_cds, "\\.\\.")) %>%  
              separate(temp_cds, into = c("cds_start", "cds_end"), sep = "\\..", convert = TRUE)
            # assembly_gap
            feature_location <- feature_location %>%
              filter(str_detect(assembly_gap_location, "\\.\\.")) %>%  
              separate(assembly_gap_location, into = c("gap_start", "gap_end"), sep = "\\..", convert = TRUE)
            
            # SVP0300; mRNA/CDS partially overlaps assembly_gap; general report b9; /data/temp_svp/warning_partial_overlap.b9 # v.1.91 
            partial_overlap <- feature_location[(feature_location$gap_start >= feature_location$cds_start & feature_location$gap_start <= feature_location$cds_end) |
                                                  (feature_location$gap_end >= feature_location$cds_start & feature_location$gap_end <= feature_location$cds_end) ,]
            if (nrow(partial_overlap) > 0) {
              partial_overlap <- transform(partial_overlap, 
                                           source = ann_file2fix, 
                                           warning = "warning",
                                           rule = "SVP0300")
              cols_order <- c("source", "warning", "rule", "Entry", "type", "cds_location")
              partial_overlap <- partial_overlap[, cols_order]
              
              write.table(partial_overlap, "/data/temp_autofix/warning_partial_overlap.b9", row.names=F, col.names=F, quote=F, sep="\t") 
            }

            # find overlapped; condition 1: gap is completely within CDS 
            feature_location <- feature_location[(feature_location$gap_start >= feature_location$cds_start & feature_location$gap_start <= feature_location$cds_end) &
                                                   (feature_location$gap_end >= feature_location$cds_start & feature_location$gap_end <= feature_location$cds_end),]
            ...
            if(len_feature_location > 0) {
              ...
              # v1.9
              fl_ncol <- ncol(feature_location)
              feature_location <- feature_location[ ,c(fl_ncol, 1:(fl_ncol-1))]
          ...
          print("BioSample exists but is incorrect.")
          log_file$biosample_id_ann <- "Missing_value" # Incorrect
        }
